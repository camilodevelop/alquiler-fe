import Link from "next/link";
import { Plus, Building2 } from "lucide-react";
import { Button } from "@/components/ui";
import {
  listPropiedadesAction,
  listPropiedadesCiudadesAction,
} from "@/app/actions/propiedades";
import { PropiedadesPageClient } from "./propiedades-page-client";

export default async function PropiedadesPage() {
  const [{ data, error }, { data: ciudades }] = await Promise.all([
    listPropiedadesAction(),
    listPropiedadesCiudadesAction(),
  ]);

  const hasPropiedades = data.length > 0;

  return (
    <div className="space-y-8">
      <header className="relative overflow-hidden rounded-2xl border border-gray-200/80 bg-gradient-to-br from-white via-brand-50/30 to-white px-6 py-8 sm:px-8 sm:py-10 shadow-sm">
        <div
          className="pointer-events-none absolute -right-8 -top-8 h-40 w-40 rounded-full bg-brand-200/30 blur-3xl"
          aria-hidden
        />
        <div className="relative flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
          <div className="flex gap-4">
            <div className="hidden sm:flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-brand-600 text-white shadow-lg shadow-brand-600/25">
              <Building2 size={28} strokeWidth={1.75} />
            </div>
            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-brand-700 mb-1">
                Cartera de inmuebles
              </p>
              <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 tracking-tight">
                Propiedades
              </h1>
              <p className="text-sm text-gray-600 mt-2 max-w-xl leading-relaxed">
                Gestiona ubicaciones, precios y estados de alquiler. Filtra por ciudad, revisa
                ingresos y accede al detalle de cada inmueble en un clic.
              </p>
            </div>
          </div>
          {hasPropiedades ? (
            <Link
              href="/dashboard/propiedades/nueva"
              className="self-start lg:self-center shrink-0"
            >
              <Button variant="primary" size="md" className="gap-2 shadow-md shadow-brand-600/20">
                <Plus size={16} />
                Nueva propiedad
              </Button>
            </Link>
          ) : null}
        </div>
      </header>

      {error && (
        <div className="rounded-xl bg-amber-50 border border-amber-200 px-4 py-3 text-sm text-amber-800">
          {error}. Comprueba la conexión con Supabase y que la migración esté aplicada.
        </div>
      )}

      <PropiedadesPageClient initialData={data} ciudades={ciudades} />
    </div>
  );
}
