-- Módulo contable — ejecutar en Supabase SQL Editor
-- Requiere: propiedades, inquilinos, contratos, mantenimiento (06–09)

-- (contenido idéntico a supabase/migrations/20260522000000_movimientos_financieros.sql)
...
create table if not exists public.movimientos_financieros (
  ...
  constraint movimientos_categoria_ingreso_check check (
    tipo <> 'ingreso'
    or categoria in (
      'pago_arriendo', 'deposito_fianza', 'administracion_comunidad',
      'servicios_publicos', 'penalizacion_mora', 'otro_ingreso'
    )
  ),
  constraint movimientos_categoria_gasto_check check (
    tipo <> 'gasto'
    or categoria in (
      'reparacion', 'mantenimiento', 'servicios_publicos', 'administracion_comunidad',
      'impuestos', 'seguro', 'limpieza', 'comision', 'publicidad', 'reforma', 'otro_gasto'
    )
  ),
...
create policy "movimientos_select" on public.movimientos_financieros
  for select to authenticated using (public.user_can_access_propiedad(propiedad_id));
...
create policy "movimientos_delete" on public.movimientos_financieros
  for delete to authenticated using (public.user_can_access_propiedad(propiedad_id));
