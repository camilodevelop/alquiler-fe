-- ============================================================
-- MÓDULO CONTRATOS — ejecutar en Supabase Dashboard → SQL Editor
-- Requiere: auth, profiles, gestor_propietario, propiedades, inquilinos,
--           user_can_access_propiedad, user_can_access_inquilino, set_updated_at()
-- ============================================================

-- Tablas primero (la función user_can_access_contrato requiere que exista contratos)
...
create table if not exists public.contrato_firmas (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint contrato_firmas_rol_check check (rol in ('administrador', 'inquilino')),
  constraint contrato_firmas_estado_check check (estado in ('pendiente', 'firmado')),
...
-- Helper: acceso a contrato vía propiedad (después de crear la tabla contratos)
...
create policy "contrato_tipos_select" on public.contrato_tipos for select
  using (public.user_can_access_inquilino(owner_id));
create policy "contrato_tipos_insert" on public.contrato_tipos for insert
  with check (public.user_can_access_inquilino(owner_id));
create policy "contrato_tipos_update" on public.contrato_tipos for update
  using (public.user_can_access_inquilino(owner_id))
  with check (public.user_can_access_inquilino(owner_id));
create policy "contrato_tipos_delete" on public.contrato_tipos for delete
  using (public.user_can_access_inquilino(owner_id));
...
create policy "contratos_insert" on public.contratos for insert
  with check (
    ...
    and exists (
      ...
      where i.id = inquilino_id and public.user_can_access_inquilino(i.owner_id)
    )
    and exists (
      select 1 from public.contrato_tipos t
      where t.id = tipo_contrato_id and public.user_can_access_inquilino(t.owner_id)
    )
  );
...
create policy "contrato_firmas_all" on public.contrato_firmas for all
...
create policy "contrato_historial_all" on public.contrato_historial for all
  using (public.user_can_access_contrato(contrato_id))
  with check (public.user_can_access_contrato(contrato_id));
